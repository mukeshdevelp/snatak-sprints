...
**IP used in this doc:** DB server **10.0.1.25** (private). Bastion has a **public IP** (use your own).
...
```bash
# Download and run the ScyllaDB server installer (adds repo and installs ScyllaDB)
curl -sSf get.scylladb.com/server | sudo bash
# Accept defaults if prompted; wait for install to complete
```
...
# Set listen_address and rpc_address to 10.0.1.25 (see table above), then save and exit (:wq)
```
...
If there is a `seeds:` list, set it to the same IP, e.g. `10.0.1.25`. Save and exit (`:wq` in vi).
...
# Optional: follow logs (Ctrl+C to exit)
